...
class Cache { //**BasicCache** 2개의 정수를 검색하고 사칙연산하는 프로그램. 
			//빈 공간이 없을 때 데이터를 무작위로 선정하고 해당 위치에 저장한다. 그리고 덧셈, 뺄셈, 곱셈, 나눗셈 사칙연산을 추가했다.
private:
...
public:
	...
	void in_cache2(int data, bool isRegister1)
	{
		if (check_cache2(data)) { //캐시2에 데이터가 있으면 가져와서 레지스터에 저장
			if (l1 >= 5) { // l1 인덱스 초과
				l1 = rand() % 5;
				L1[l1] = data;
				l1 += 5; // if 조건에 만족해야 난수를 생성하기 때문에 조건에 맞추기 위해 알맞은 값을 더한다.
			}
			else {
				L1[l1] = data;
				l1++;
	...
	void in_cache3(int data, bool isRegister1)
	{
		...
		else in_ram(data, isRegister1); //캐시3에 데이터가 없으면 램을 검사하여 캐시에 적재하는 함수 호출
	}
	...
	void in_hdd(int data, bool isRegister1)
	{
		...
		if (check_hdd(data)) { //하드디스크를 검사해서 데이터가 있으면 가져와서 램에 저장 -> 캐쉬에 저장 -> 레지스터에 저장
			if (r >= 500) { //ram 인덱스 초과
				r = rand() % 500;
				RAM[r] = data;
				r += 500; // if 조건에 만족해야 난수를 생성하기 때문에 조건에 맞추기 위해 알맞은 값을 더한다.
	...
	void show_calculation_result(int cal) {
		if (cal == 1)
			cout << "두 정수의 덧셈 결과는 " << R1 + R2 << " 이다." << endl;
		else if (cal == 2)
			cout << "두 정수의 뺄셈 결과는 " << R1 - R2 << " 이다." << endl;
		else if (cal == 3)
			cout << "두 정수의 곱셈 결과는 " << R1 * R2 << " 이다." << endl;
		else if (cal == 4)
			cout << "두 정수의 나눗셈 결과는 " << static_cast<double>(R1) / R2 << " 이다." << endl;
	}
};
...
{
	Cache c1;
	int input1, input2, count = 0, calculation;
	...
	while (true) {
		...
		cout << "연산 하고자 하는 사칙 연산의 숫자를 입력하시오. (덧셈 : 1, 뺄셈 : 2, 곱셈 : 3, 나눗셈 : 4)" << endl;
		cin >> calculation;

		c1.show_calculation_result(calculation);
